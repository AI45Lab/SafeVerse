...
class ActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # print(f'[DEBUG] {repeat_times=}') # 我要检查一下这里的tuple报错是否repeat_times输出的
        output = output.to("cpu")
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        with init_context(), warnings.catch_warnings():
            ...
            print(f'[DEBUG] begin to load critic with qwen2.5vl')
            critic_module = Qwen2_5_VLForTokenClassification.from_pretrained(pretrained_model_name_or_path=local_path,
                                                                        torch_dtype=torch_dtype,
                                                                        config=critic_model_config,
                                                                        attn_implementation='flash_attention_2',
                                                                        trust_remote_code=trust_remote_code)
            print(f'[DEBUG] load with success!')

            
            # if "Qwen2.5-VL" in local_path or "era" in local_path or "Qwen" in local_path or "alfred" in local_path or "man" in local_path:
            #     from verl.models.transformers.modeling_qwen_2_5_vl_patch import Qwen2_5_VLForTokenClassification
            #     print(f'[DEBUG] begin to load critic with qwen2.5vl')
            #     critic_module = Qwen2_5_VLForTokenClassification.from_pretrained(pretrained_model_name_or_path=local_path,
            #                                                                 torch_dtype=torch_dtype,
            ...
            #     print(f'[DEBUG] load with success!')
            # else:
            #     print(f'[DEBUG] citic w/o qwen2.5vl')
            ...
            #### support freeze vision tower
            if self.config.model.freeze_vision_tower:
                for p in critic_module.visual.parameters():
                    p.requires_grad = False
                for p in critic_module.visual.merger.parameters():
        ...
        # 尝试动态检测层结构
        target_layer_cls = None
        try:
            if hasattr(critic_module, "model") and hasattr(critic_module.model, "layers"):
                 if len(critic_module.model.layers) > 0:
                     target_layer_cls = type(critic_module.model.layers[0])
            elif hasattr(critic_module, "layers") and len(critic_module.layers) > 0:
                 target_layer_cls = type(critic_module.layers[0])
            ...
            if target_layer_cls:
                ...
                if self.rank == 0:
                    print(f"✅ [FSDP Policy] Resolved wrapper policy for: {target_layer_cls.__name__}")
        except Exception:
            pass
        ...
        critic_module = FSDP(
            critic_module,
            param_init_fn=init_fn,
            use_orig_params= True if self.config.model.freeze_vision_tower else False,
            auto_wrap_policy=auto_wrap_policy,
            device_id=torch.cuda.current_device(),
    ...
    def init_model(self):
        ...
        # =================================================================
        # [PATCH] 手动设置梯度累积步数 (修复 AttributeError)
        # =================================================================
        # update_critic 需要这个属性来计算平均 loss
        self.critic.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
        print(f'[INFO] in fsdp_workers.py, {self.critic.gradient_accumulation=}')
        # =================================================================
