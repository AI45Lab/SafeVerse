...
class DataProto:
    ...
    def chunk(self, chunks: int) -> List["DataProto"]:
        """
        Split the batch among dim=0 into chunks (will be as equal as possible).
        The meta_info is passed to each DataProto after split.
        ...
        Returns:
            List[DataProto]: a list of DataProto after splitting
        """
        # Split batch (torch.Tensor or None)
        if self.batch is not None:
            import torch
            batch_size = self.batch.shape[0]
            print(f'[INFO] {batch_size=}, {chunks=}')
            sizes = [batch_size // chunks] * chunks
            for i in range(batch_size % chunks):
                sizes[i] += 1
            batch_lst = torch.split(self.batch, sizes, dim=0)
        else:
            batch_lst = [None for _ in range(chunks)]

        # Split non_tensor_batch (np.ndarray)
        non_tensor_batch_lst = [{} for _ in range(chunks)]
        for key, val in self.non_tensor_batch.items():
            assert isinstance(val, np.ndarray)
            non_tensor_lst = np.array_split(val, chunks)  # array_split handles unequal
            for i in range(chunks):
                non_tensor_batch_lst[i][key] = non_tensor_lst[i]
    ...
    # 这里的repeat_times支持一个list的输入
    def repeat(self, repeat_times=2, interleave=True):
        """
        Repeat the batch data a specified number of times.

        Args:
            repeat_times (int or list): Number of times to repeat each element. 
                If int, all elements repeat the same times. 
                If list, each element repeats according to its corresponding value.
            interleave (bool): Whether to interleave the repeated data.

        Returns:
            DataProto: A new DataProto with repeated data.
        """
        # Convert int to list for uniform processing
        is_list = isinstance(repeat_times, (list, tuple))
        
        if self.batch is not None:
            # 这里也要看下是不是repeat之前就pop空了
            print(f'[DEBUG] in protocal.py, {self.batch.keys()=}')
            if is_list:
                # Use repeat_interleave with repeats parameter
                repeated_tensors = {
                    key: torch.repeat_interleave(tensor, torch.tensor(repeat_times), dim=0)
                    for key, tensor in self.batch.items()
                }
                new_batch_size = sum(repeat_times)
            else:
                ...
                new_batch_size = self.batch.batch_size[0] * repeat_times
            ...
            repeated_batch = None
        # 这里需要检查一下是否在repeat之前就pop空了
        # print(f'[DEBUG] in protocal, {repeated_batch=}')
        repeated_non_tensor_batch = {}
        for key, val in self.non_tensor_batch.items():
            print(f'[DEBUG] in protocal.py, {self.non_tensor_batch.keys()=}')
            if is_list:
                # Repeat each element according to list
                repeated_non_tensor_batch[key] = np.repeat(val, repeat_times, axis=0)
            else:
                if interleave:
                    repeated_non_tensor_batch[key] = np.repeat(val, repeat_times, axis=0)
                    ...
                    repeated_non_tensor_batch[key] = np.tile(val, (repeat_times,) + (1,) * (val.ndim - 1))
        # print(f'[DEBUG] in protocal, {repeated_non_tensor_batch=}')
        return DataProto(
            batch=repeated_batch,
            non_tensor_batch=repeated_non_tensor_batch,
            meta_info=self.meta_info,
