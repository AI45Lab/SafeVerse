...
openai_api_base_list = [
    ...
    os.environ.get("LLM_AS_A_JUDGE_BASE", "http://100.100.94.218:20001/v1"), # 这里似乎没办法读到
]
...
def compute_score(predict_str: str, ground_truth: str, extra_info=None) -> float:
    print(f"[DEBUG compute score] Using none reasoning.")
    with open(f"compte_score.txt", "w") as f:
        f.write("[DEBUG compute score] Using none reasonging.")
    ...
    print(f"[DEBUG compute score, none reasoning] {acc_reward=}, {format_reward=}, {tool_reward=}\n")
    # reward 1
...
def compute_common_reasoning(predict_str: str, ground_truth: str, extra_info=None) -> float:
    print(f"[DEBUG compute score] Using reasoning func.")
    with open(f"compte_score.txt", "w") as f:
        f.write("[DEBUG compute score] Using reasonging fucn.")
    is_format_error = False
    # predict_str = "<think>" + predict_str
    count_think_1 = predict_str.count("<think>")
    count_think_2 = predict_str.count("</think>")
    ...
    tool_reward_base = 1.0 if count_vision_1 > 0 else 0.0
    tool_reward = 1.0 if count_vision_1 > 0 and acc_reward > 0.5 else 0.0
    format_reward = -1.0 if is_format_error else 0.0 # format只讲惩罚
    total_reward = 0.8 * acc_reward + 0.2 * format_reward + 1.2 * tool_reward
    print(f"[DEBUG compute score, reasoning] {acc_reward=}, {format_reward=}, {tool_reward=}\n")
    print(f"[DEBUG compute score, reasonging] {total_reward=}\n\n")
