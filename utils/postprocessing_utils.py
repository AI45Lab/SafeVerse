...
def parametrize_mesh(vertices, faces):
    save_dir = "/mnt/shared-storage-user/steai-share/zhanglechao/TRELLIS/tmp_data"
    os.makedirs(save_dir, exist_ok=True)

    import time
    task_id = str(int(time.time() * 1000))  # 毫秒级

    v_path = f"{save_dir}/{task_id}_vertices.npy"
    f_path = f"{save_dir}/{task_id}_faces.npy"
    done_flag = f"{save_dir}/{task_id}.done"

    # 写输入文件
    np.save(v_path, vertices)
    np.save(f_path, faces.astype(np.int32))

    print(f"[Intel] Task {task_id} saved, waiting for AMD...")

    # 等待 AMD 输出
    while not os.path.exists(done_flag):
        time.sleep(0.2)

    print(f"[Intel] Task {task_id} done, loading results...")

    vertices_new = np.load(f"{save_dir}/{task_id}_vertices_uv.npy")
    faces_new = np.load(f"{save_dir}/{task_id}_faces_uv.npy")
    uvs = np.load(f"{save_dir}/{task_id}_uvs.npy")

    for fn in [
        f"{task_id}_vertices_uv.npy",
        f"{task_id}_faces_uv.npy",
        f"{task_id}_uvs.npy",
        f"{task_id}.done"
    ]:
    ...
    print(f"[Intel] Cleaned task {task_id}.")
    return vertices_new, faces_new, uvs
